...
if(isset($_POST['save'])){
    ...
    if(!isset($_POST['id']) || $_POST['id']==''){
        ...
        if($errors=='Trạng thái:'){
            ...
            $image = $target;
            $sql="insert into brands (name, logolink, created_at, updated_at) values (N'".$name."',N'".basename($image)."','".$created_at."','".$updated_at."')";
            execute($sql);
            $errors=$errors.'<br>Đã lưu';
            $_SESSION['notify']=$errors;
            $sqlback="select * from brands where name='".$name."'";
    ...
    // Khi chỉnh sửa
        ...
        if($errors=='Trạng thái:'){
            $sql="update brands set name=N'".$name."', logolink= N'".$name.basename($image)."', updated_at='".$updated_at."' where id =".$id;
            execute($sql);
            $errors=$errors.'<br>Đã lưu';
            $_SESSION['notify']=$errors;
